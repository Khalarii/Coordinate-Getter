...
if (current_url.includes('tribalwars') && current_url.includes('village=') && current_url.includes('mode=units')) {
    ...
    if (units_table != null) {
        var village_info_offset = 0;
        var id_cell = 0;

        for (var i = 1; i < units_table.length; i++) {
            var row = units_table[i];

            if (row.cells[id_cell].firstChild.data != 'Na Aldeia' && row.cells[id_cell].firstChild.data != 'fora' && row.cells[id_cell].firstChild.data != 'em trânsito' && row.cells[id_cell].firstChild.data != 'total' && row.cells[id_cell].firstChild.data != '<textarea></textarea>') {
                var j = row.cells[id_cell];
                var village_units = [];
